...
`portfolio_surveillance_v1` has a fail-closed conditional schema and delegation seam, but the production `remote-cli` does not inject its dedicated workflow. Production capabilities and `abl_start_job` therefore continue to expose only the four operations above. Do not enable the fifth operation until repository-backed publication authority, access-policy preflight/materialization, and durable v4 execution/recovery are composed.
