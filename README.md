...
`portfolio_surveillance_v1` has a fail-closed conditional schema and separate delegation seam, but the production `remote-cli` does not inject its dedicated workflow. Production capabilities and `abl_start_job` therefore continue to expose only the four operations above. The repository-backed publication verifier, metadata-only preflight, post-policy materializer, and dedicated v4 lifecycle are additive foundations, not production enablement. Do not expose the fifth operation until trusted modern snapshot/certification writers, active mapping/runtime-bundle authorities, durable runtime composition, and opaque-handle routing for status/result/cancel are implemented and reviewed.
