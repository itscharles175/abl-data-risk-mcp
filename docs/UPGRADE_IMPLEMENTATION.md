...
- production `DatasetSnapshotV2` and certification-evidence repositories, a governed durable dataset/scope binding, repository-backed publication authority, unique effective access-policy selection, metadata-only preflight, and post-policy artifact materialization;
- registration of the pure portfolio-surveillance operation in the signed durable workflow/worker and remote MCP surface with backward-compatible v4 envelope/result recovery; `buildRemoteServer` conditionally supports the fifth operation only when a dedicated workflow is injected, and production `remote-cli` deliberately omits it;
