...
4. The sidecar, definition/longitudinal/metric-evidence, publication-reader, metadata-preflight, post-policy-materialization, dedicated v4 durability, and modern capture/certification foundation slices are closed at the local/conformance boundary. Begin the next production slice with lifecycle-backed source, scope, control, runtime, methodology, dimension, and FX authorities plus certification artifact staging; do not enable the remote seam.
