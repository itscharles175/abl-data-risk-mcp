...
4. The sidecar, definition/longitudinal/metric-evidence, publication-reader, metadata-preflight, post-policy-materialization, dedicated v4 durability, and modern capture/certification foundation slices are closed at the local/conformance boundary. An IDs-only lifecycle-backed source-delivery registration service now verifies effective source-contract and dataset-binding versions before catalog persistence, but it is not yet composed into a production connector/runtime. Begin the next production slice by wiring that service and adding lifecycle-backed control, runtime, methodology, dimension, and FX authorities plus certification artifact staging; do not enable the remote seam.
...
Production enablement remains intentionally absent. Local services now create the modern capture and certification records under injected authorities, and certification uses the exact receipt/delivery/facility-bound selector, but the checked-in production runtime does not compose those ports and the available source/control registration paths are trusted-import foundations rather than independently approved lifecycle authority. Required before enablement: governed source/scope, DQ, reconciliation, methodology, dimension, compiler-compatibility, and FX authorities; staged/orphan-recoverable artifact publication; historical activation-at-use replay; concrete production adapters; operator runtime composition; and a composite opaque-handle router for the fifth operation. Production `remote-cli` omits the optional portfolio-surveillance workflow and continues to advertise and execute only the four legacy governed operations.
...
1. Replace trusted source/scope and certification-set imports with durable maker/checker lifecycle adapters. Govern DQ, reconciliation, methodology, content-addressed dimensions, compiler compatibility, and FX definitions/rates; bind every executable reference and approval event into certification evidence.
